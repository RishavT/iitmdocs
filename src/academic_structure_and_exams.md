---
title: Academic Structure and Exams
description: Covers refund policies, fee rules, and key considerations before registering for the IIT Madras BS program.
covers:
  - Application and reattempt fee refund rules
  - Non-refundable fee conditions
  - Course fee refund guidance
  - Fee structure by category
  - Important considerations before payment
  - Contact for fee-related issues
sample_queries:
  - Is the qualifier application fee refundable?
  - Will I get a refund if I miss the exam?
  - What is the reattempt fee refund policy?
  - Can I get my course fee refunded?
  - What should I check before paying fees?
  - Who do I contact for fee issues?
---

# Refund Policy

## Application Fee - Non-Refundable
The Qualifier exam application fee is NON-REFUNDABLE under ANY circumstances.

This includes:
- Unable to write the exam
- Absent from exam
- Failed the exam
- Changed mind about joining
- Medical emergencies
- Personal reasons

## Reattempt Fee - Non-Refundable
The reattempt fee is also non-refundable.

## Course Fee Refunds
Course registration fees follow program refund policy. Contact support@study.iitm.ac.in for specific cases.

## Before Paying
Consider the following before registering:
- Are you sure about joining?
- Can you commit to the schedule?
- Do you have time for coursework?
- Can you attend exams at selected cities?

## Fee Structure Summary
| Fee Type | Amount | Refundable |
|----------|--------|------------|
| Application Fee (General/OBC) | Rs 4000 | No |
| Application Fee (SC/ST/PwD) | Rs 2000 | No |
| Reattempt Fee (General/OBC) | Rs 2000 | No |
| Reattempt Fee (SC/ST/PwD) | Rs 1000 | No |

## Getting Money Back
You will NOT get your money back if:
- You don't want the course anymore
- You can't write the exam
- You are absent
- You fail

## Contact for Fee Issues
support@study.iitm.ac.in

---

Tags: refund policy, non refundable fees, application fee rules, reattempt fee rules, course fee refund, fee structure, general obc fee 4000, sc st pwd fee 2000, reattempt fee 2000 1000, no refund conditions, absent exam no refund, failed exam no refund, payment considerations, before paying fees, contact support for refund, fee related queries