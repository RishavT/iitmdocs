---
title: qualifier_preparation
description: Qualifier process preparation details, including 4-week coursework, Week-1 content access, weekly releases, assignments, and exam timing after four weeks.
covers:
  - 4-week Qualifier process
  - AE Week-1 Foundation level courses
  - Videos, tutorials, assignments, and transcripts
  - Weekly graded assignment submission
  - Qualifier exam after four weeks
sample_queries:
  - How long is the Qualifier process?
  - Which courses are provided in Week 1 for AE qualifier preparation?
  - What learning materials are included in the qualifier content?
  - Do I need to submit graded assignments every week?
  - What content is the qualifier exam based on?
---

# Content

## Qualifier process and coursework

All regular entry applicants must go through the 4-week Qualifier process to earn admission to the Foundation Level.

BS in Aeronautics and space technology (AE): On the start date of the Qualifier process Week-1, access to the content for the four Foundation level courses English 1, Mathematics-1, Electronic Systems Thinking and Circuits, and Introduction to C Programming will be provided.

The content that includes videos, tutorials, practice/graded assignments and text transcripts is released week wise.

The content will be made live with an announcement on the portal, and an alert through email.

Every week the graded assignment must be submitted in each course.

This allows candidates to explore the learning material and understand the structure of the program before attempting the exam.

At the end of the 4 weeks, a qualifier exam will be conducted for eligible candidates based on the content covered in the 4 weeks of study.

---

Tags:
- qualifier preparation
- 4-week qualifier process
- Week-1 content
- AE qualifier courses
- Foundation courses
- graded assignments
- videos tutorials transcripts
- qualifier exam preparation
