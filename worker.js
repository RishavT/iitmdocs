...
- "फीस कितनी है" → "fee cost structure payment foundation diploma degree fees [LANG:hindi]"`;
  ...
  try {
    ...
    // ================= TOKEN USAGE (QUERY REWRITE) =================
    const rewriteUsage = result.usage
      ? {
      ...
      : null;
    console.log(
      "[DEBUG] Rewrite LLM tokens | prompt:",
    ...
    return {
      ...
      source: "llm",
      usage: rewriteUsage, // return token usage to caller
    };
  } catch (error) {
...
async function answer(request, env) {
  ...
  const logContext = {
    ...
    latency_ms: null,
    error: null,
    // Adding the following for cost analysis
    total_input_tokens: null,   // Total tokens sent to LLM (prompt + context + history)
    total_output_tokens: null,  // Total tokens generated by LLM
    cached_prompt_tokens: null,      // Prompt tokens served from cache (discounted)
    non_cached_prompt_tokens: null,  // Prompt tokens billed at full rate
  };
    ...
    async start(controller) {
      try {
        ...
        const {
          ...
          source: querySource,
          usage: rewriteUsage,
        } = await rewriteQueryWithSource(question, env);
        logContext.rewritten_query = searchQuery;
        logContext.query_source = querySource;
        // ================= TOKEN ACCUMULATION =================
        // Add rewrite-query token cost to logContext
        if (rewriteUsage && logContext) {
          logContext.total_input_tokens =
            (logContext.total_input_tokens ?? 0) + rewriteUsage.input_tokens;

          logContext.total_output_tokens =
            (logContext.total_output_tokens ?? 0) + rewriteUsage.output_tokens;

          logContext.cached_prompt_tokens =
            (logContext.cached_prompt_tokens ?? 0) +
            (rewriteUsage.cached_prompt_tokens ?? 0);

          logContext.non_cached_prompt_tokens =
            (logContext.non_cached_prompt_tokens ?? 0) +
            (rewriteUsage.input_tokens - (rewriteUsage.cached_prompt_tokens ?? 0));
        }
        console.log(
          "[DEBUG] Accumulated tokens after rewrite | input:",
          logContext.total_input_tokens,
          "output:",
          logContext.total_output_tokens
        );
        ...
        await answerResponse.body.pipeTo(
          new WritableStream({
            ...
            close: () => {
              ...
              logContext.latency_ms = Date.now() - startTime;
              console.log(
                "[DEBUG] FINAL token totals | input:",
                logContext.total_input_tokens,
                "output:",
                logContext.total_output_tokens
              );
...
Current date: ${new Date().toISOString().split("T")[0]}.${contextNote}`;
  ...
  // ================= TOKEN USAGE LOGGING (ANSWER LLM) =================
  // Accumulate token usage from the main answer LLM call
  if (logContext && result.usage) {
    ...
    logContext.total_input_tokens =
      (logContext.total_input_tokens ?? 0) + promptTokens;

    logContext.total_output_tokens =
      (logContext.total_output_tokens ?? 0) + completionTokens;

    logContext.cached_prompt_tokens =
      (logContext.cached_prompt_tokens ?? 0) + cachedTokens;

    logContext.non_cached_prompt_tokens =
      (logContext.non_cached_prompt_tokens ?? 0) + (promptTokens - cachedTokens);
  }
  ...
  /*
  cost =
    (non_cached_prompt_tokens × full_prompt_rate)
  ...
  if (hasRaahat) {
    ...
    if (otherStatementCount > 2) {
      ...
      let isOtherChunkValid = await checkResponse({ response: otherChunk, context, history: validatedHistory, env, logContext });
      ...
      if (!isOtherChunkValid && validatedHistory.length > 0) {
        ...
        isOtherChunkValid = await checkResponse({ response: otherChunk, context, history: [], env, logContext });
      }
    ...
    let isFactuallyCorrect = await checkResponse({ response: answerText, context, history: validatedHistory, env, logContext});
    ...
    if (!isFactuallyCorrect && validatedHistory.length > 0) {
      ...
      isFactuallyCorrect = await checkResponse({ response: answerText, context, history: [], env, logContext});
 ...
 */
async function checkResponse({ response, context, history = [], env, logContext = null }){
...
Output your fact-check result as JSON:`;
  ...
  console.log('[DEBUG] Fact-check tokens | prompt:', result.usage?.prompt_tokens, 
